...
public:
    ...
    */
    ...
    int16_t raw(const uint8_t *pbtTx, uint16_t szTx, uint8_t *pbtRx, uint16_t szRxLen, int timeout = 1000);
    ...
    uint8_t ntag2xx_Auth (const uint8_t * password, uint8_t * pack);
    uint8_t ntag2xx_Halt ();
    // FeliCa Functions
