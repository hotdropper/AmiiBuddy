...
#include "PN532.h"
#define PN532_DEBUG
#undef ARDUINO_SAMD_VARIANT_COMPLIANCE
...
bool PN532::readID(uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
    return PN532::readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, uidLength, timeout);
}

void PN532::ntag2xx_FinishedReading() {
...
uint8_t PN532::ntag2xx_WritePage (uint8_t page, uint8_t *buffer) {
    return PN532::mifareultralight_WritePage(page, buffer);
}

uint8_t PN532::ntag2xx_Auth (const uint8_t * password, uint8_t * pack)
{
    /* Prepare the command */
    ...
    /* Read the response packet */
    uint8_t resp = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer));
    PRINTV("Got auth response: ", resp);
    PRINTHEX(pn532_packetbuffer, 3);
    memcpy(pack, &pn532_packetbuffer[1], 2);
    return pn532_packetbuffer[0];
}

uint8_t PN532::ntag2xx_Halt ()
{
    /* Prepare the command */
    pn532_packetbuffer[0] = 0x50;
